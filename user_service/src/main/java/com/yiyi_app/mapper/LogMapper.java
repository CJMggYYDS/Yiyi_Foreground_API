...
public interface LogMapper {
    ...
    @Insert("insert into shareclothes.log(uid, timestamp, action, itemId) values (#{uid}, #{timestamp}, #{action}, #{itemId})")
    ...
    @Delete("delete from shareclothes.log where uid=#{uid} and action=#{action} and itemId=#{itemId}")
    int deleteAction(@Param("uid") String uid, @Param("action") int action, @Param("itemId") String itemId);

    @Select("select itemId, timestamp from shareclothes.log where uid=#{uid} and action=#{action} order by timestamp desc")
